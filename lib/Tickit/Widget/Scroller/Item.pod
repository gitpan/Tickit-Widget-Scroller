...
=head2 $item->render( $rc, %args )

Renders the item to the given L<Tickit::RenderContext>. C<%args> will contain
the following keys:
...
ignore this if they wish, as clipping will be performed by the RenderContext.
